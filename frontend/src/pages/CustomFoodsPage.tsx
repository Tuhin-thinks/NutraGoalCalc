    ...
    <div className="flex h-[calc(100dvh-4rem)] flex-col">
      ...
      <header className="flex h-16 shrink-0 items-center justify-between border-b border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-4">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={onBack} className="text-neutral-500 dark:text-neutral-400">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {loading ? (
          ...
          <p className="py-8 text-center text-sm text-neutral-400 dark:text-neutral-500">No foods found.</p>
        ) : (
            ...
            {filtered.map((food) => {
              ...
              return (
                <div
                  key={food.id}
                  className={cn(
                    "flex items-center gap-3 rounded-md border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2.5",
                    "border-l-4",
                    color.border,
                  )}
                >
                  <div className="min-w-0 flex-1">
                    <p className="flex items-center gap-2 truncate text-sm font-medium text-neutral-900 dark:text-neutral-100">
                      {food.name}
                      {food.is_custom && (
                        <span className="rounded bg-neutral-100 dark:bg-neutral-800 px-1.5 py-0.5 text-[10px] font-medium text-neutral-500 dark:text-neutral-400">custom</span>
                      )}
                    </p>
                  ...
                  <div className="flex gap-1">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-neutral-400 dark:text-neutral-500 hover:text-red-500" title="Delete" onClick={() => handleDelete(food.id, food.name)}>
      ...
      {(showForm || editData) && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/30 pt-12">
          <div className="w-full max-w-lg rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-6 shadow-xl dark:shadow-black/20">
