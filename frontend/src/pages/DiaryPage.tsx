    ...
    <div className="mx-auto flex h-[calc(100dvh-4rem)] max-w-5xl flex-col px-4 py-4 md:flex-row md:gap-6">
      ...
      <div className="flex shrink-0 flex-col md:w-[420px]">
        <div className="mb-4 flex items-center justify-between">
          <button
            onClick={onBack}
            className="rounded-md border border-neutral-200 dark:border-neutral-700 px-2.5 py-1 text-xs font-medium text-neutral-500 dark:text-neutral-400 hover:border-neutral-300 dark:hover:border-neutral-600 hover:text-neutral-700 dark:hover:text-neutral-300"
          >
            ← Back to Tracker
          </button>
        </div>
        <div className="flex items-center justify-between rounded-md bg-white dark:bg-neutral-900 px-3 py-2 shadow-sm dark:shadow-black/20">
          <button onClick={prevMonth} className="rounded p-1 text-neutral-400 dark:text-neutral-500 hover:text-neutral-600 dark:hover:text-neutral-400">
            <ChevronLeft className="h-5 w-5" />
          </button>
          <span className="text-base font-semibold text-neutral-800 dark:text-neutral-200">
            {MONTHS[month]} {year}
          </span>
          <button onClick={nextMonth} className="rounded p-1 text-neutral-400 dark:text-neutral-500 hover:text-neutral-600 dark:hover:text-neutral-400">
            <ChevronRight className="h-5 w-5" />
          </button>
        </div>
        <div className="mt-3 rounded-md bg-white dark:bg-neutral-900 p-3 shadow-sm dark:shadow-black/20">
          <CalendarGrid
            year={year}
            month={month}
      ...
      <div className="mt-4 flex min-h-0 flex-1 flex-col md:mt-0 md:overflow-y-auto">
        ...
        {selectedItems.length > 0 && (
          <div className="mt-4 rounded-md border border-dashed border-neutral-200 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-900/50 p-3 text-center text-xs text-neutral-400 dark:text-neutral-500">
